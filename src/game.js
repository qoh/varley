...
const Game = class Game {
	...
	callback(name, ...args) {
		if (this.serverEnvironment.hasOwnProperty(name)) {
			this.serverEnvironment[name](...args)
		}
	}
	...
	disconnect(ws) {
		...
		if (index > -1) {
			...
			this._initVars()
			this.callback('disconnect', player)
		}
	}
	...
	message(ws, msg) {
		...
		if (json.press) {
			...
			this.callback('press', player, json.press)
		}

		if (json.release) {
			player.keys[json.release] = false
			this.callback('release', player, json.release)
		}

		for (let k in json) {
			this.callback(k, player, json[k])
		}
	}
	...
	tick() {
		...
		this.players.forEach(player => this.callback('playertick', player))
	...
	_initVars() {
		this.serverEnvironment.pub = this.pub
		this.serverEnvironment.players = this.players
		this.serverEnvironment.playSound = this._playSound
	}
